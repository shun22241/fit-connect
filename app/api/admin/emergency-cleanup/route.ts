...
export async function POST(request: Request) {
  try {
    ...
    results.steps.push(
      `Found ${usersWithEmail.length} auth users with email: ${email}`,
    )
    ...
    for (const user of usersWithEmail) {
      ...
      if (!user.email_confirmed_at && user.confirmed_at) {
        corruptions.push(
          'Email confirmed timestamp missing but confirmed_at exists',
        )
      }
      ...
      if (ageInDays > 30 && !user.last_sign_in_at) {
        corruptions.push(
          `User created ${Math.floor(ageInDays)} days ago but never signed in`,
        )
      }
    ...
    if (action === 'cleanup' && usersWithEmail.length > 0) {
      ...
      results.steps.push(
        `  - Browser: localStorage.clear() + sessionStorage.clear()`,
      )
    ...
    if (action === 'suggest_email') {
      ...
      results.suggested_emails.forEach((e: string) =>
        results.steps.push(`  - ${e}`),
      )
    }
