  ...
  try {
    ...
    for (const user of authUsers.users) {
      ...
      if (user.created_at && !user.last_sign_in_at) {
        const daysSinceCreated = Math.floor((new Date().getTime() - new Date(user.created_at).getTime()) / (1000 * 60 * 60 * 24))
        if (daysSinceCreated > 7) {
          userIssues.push(`User created ${daysSinceCreated} days ago but never signed in`)
        }
      }
      
      // Check for duplicate emails
      const duplicates = authUsers.users.filter(u => u.email === user.email && u.id !== user.id)
      if (duplicates.length > 0) {
        userIssues.push(`Duplicate email found: ${duplicates.length} other users`)
      }
    ...
    if (inconsistentUsers.length > 0) {
      results.issues.push(`Found ${inconsistentUsers.length} users with data inconsistencies`)
    ...
    try {
      ...
      if (error.message?.includes('email_provider_disabled')) {
        ...
        results.recommendations.push('Enable email provider in Supabase dashboard')
      } else {
    ...
    if (results.issues.length === 0) {
      ...
      results.recommendations.push('Use the cleanup tool to resolve data inconsistencies')
      results.recommendations.push('Consider implementing user data validation hooks')
    }
