...
export async function POST(request: Request) {
  try {
    ...
    results.steps.push(`Found ${authUsers?.users?.length || 0} total auth users`)
    ...
    if (authUser) {
      ...
      if (deleteAuthError) {
        results.errors.push(`Failed to delete auth user: ${deleteAuthError.message}`)
      } else {
    ...
    try {
      ...
      if (profileDeleteError) {
        results.errors.push(`Profile cleanup error: ${profileDeleteError.message}`)
      } else {
