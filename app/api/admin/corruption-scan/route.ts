  ...
  try {
    ...
    if (results.corrupted_users.length > 0) {
      results.recommendations.push(
        `Found ${results.corrupted_users.length} corrupted users`,
      )
      results.recommendations.push(
        'Use emergency cleanup tool to fix corrupted users',
      )
    }

    if (results.duplicate_emails.length > 0) {
      results.recommendations.push(
        `Found ${results.duplicate_emails.length} duplicate email addresses`,
      )
      results.recommendations.push(
    ...
    if (
      results.corrupted_users.length === 0 &&
      results.duplicate_emails.length === 0
    ) {
    ...
    results.recommendations.push(
      'Consider implementing automated cleanup for old unconfirmed users',
    )
    results.recommendations.push(
      'Set up monitoring for duplicate email detection',
    )
